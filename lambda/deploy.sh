...
aws iam attach-role-policy --role-name helloworld-lamba --policy-arn arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole
...
API_INFO=$(aws apigatewayv2 create-api --name helloworld-http-api --protocol-type HTTP --target ${FUNCTION_ARN} --query '[ApiEndpoint,ApiId]' | tr -d ',[]\" ' |tr '\n' ';')
